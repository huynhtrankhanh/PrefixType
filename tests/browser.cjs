...
(async () => {
  ...
  try {
    ...
    for (const viewport of [{ width: 393, height: 851 }, { width: 1100, height: 760 }]) {
      await run(`touch added after load, mixed pointers, handles and scrolling at ${viewport.width}px`, async () => {
        const context = await browser.newContext({ permissions: ['clipboard-read', 'clipboard-write'], viewport, deviceScaleFactor: 2 });
        const tab = await context.newPage(); tab.on('pageerror', error => errors.push(error.message)); await tab.goto(server.url);
        ...
        assert.equal(await tab.evaluate(() => navigator.maxTouchPoints), 0);
        await tab.mouse.click(point.x, point.y);
        assert.equal(await tab.locator('.selection-handle:visible').count(), 0);
        // Simulate connecting a touchscreen AFTER initialization, keeping the
        // desktop browser identity and the existing editor instance.
        await tab.evaluate(() => window.originalEditor = PrefixType.editor);
        ...
        await tab.locator('#selectionToolbar').waitFor({ state: 'visible', timeout: 2000 });
        ...
        await tab.screenshot({ path: `test-results/touch-selection-${viewport.width}.png` });
        await tab.mouse.click(point.x, point.y);
        await tab.waitForFunction(() => [...document.querySelectorAll('.selection-handle')].every(handle => handle.hidden));
        assert.equal(await tab.locator('#selectionToolbar').isVisible(), false);
        await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [point] });
        await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
        await tab.waitForFunction(() => document.querySelectorAll('.selection-handle:not([hidden])').length === 1);
        assert.equal(await tab.evaluate(() => PrefixType.editor === window.originalEditor), true);
        ...
        await tab.mouse.click(emptyPoint.x, emptyPoint.y);
        await tab.waitForFunction(() => [...document.querySelectorAll('.selection-handle')].every(handle => handle.hidden));
        await context.close();
      });
    }
