...
The canvas editor uses `EditContext` on desktop and a transparent, browser-managed textarea on Android. Other browsers without `EditContext` automatically use a visible native textarea. **Native input** also lets you switch to a textarea for assistive technology or native platform editing. Clipboard access requires a browser context that allows it, such as localhost or HTTPS.
...
Tab retains normal browser focus navigation. On the EditContext path, a complete IME composition is one undo group; other edits are individual undo steps, with up to 1,000 groups retained. Android keyboard navigation, composition, and keyboard undo/redo use the browser’s native textarea behavior.
